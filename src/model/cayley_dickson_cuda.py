...
def test_packed_correctness(verbose=True):
    """
    Test that packed weights produce correct results vs reference.
    """
    torch.manual_seed(42)
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    
    B, T, K, N = 4, 128, 256, 256
    
    # Create test inputs
    x_parts = tuple(torch.randn(B, T, K // 8, device=device, dtype=torch.bfloat16) for _ in range(8))
    w = torch.randint(-1, 2, (8, N // 8, K // 8), device=device).float()  # Ternary
    beta = torch.ones(N // 8, device=device) * 0.1
    
    # Reference (unpacked)
    y_ref = cayley_dickson_fused(x_parts, w)
    y_ref = tuple(y * beta for y in y_ref)
    
    # Pack and unpack
    w_packed = pack_ternary_weights(w)
    w_unpacked = unpack_ternary_weights(w_packed, K // 8)
    
    # Test with packed wrapper
    y_packed = cayley_dickson_packed(x_parts, w_packed, beta, K // 8)
    
    # Compare
    max_diff = max((y_p - y_r).abs().max().item() for y_p, y_r in zip(y_packed, y_ref))
    pack_matches = (w == w_unpacked).all().item()
    
    if verbose:
        print(f"Packed Weight Correctness Test:")
        print(f"  Shape: B={B}, T={T}, K={K}, N={N}")
        print(f"  Pack/Unpack matches: {pack_matches}")
        print(f"  Max output difference: {max_diff:.6f}")
        print(f"  Weight memory: {w.numel() * 4} bytes -> {w_packed.numel()} bytes ({w_packed.numel() / (w.numel() * 4) * 100:.1f}%)")
    
    passed = pack_matches and max_diff < 1e-5
    if verbose:
...
def cayley_dickson_reference(x_parts, W):
    """
    Reference implementation matching physics.py OctonionTernaryLinear.
    Used for correctness testing.
    """
    x0, x1, x2, x3, x4, x5, x6, x7 = x_parts
    Wt = [W[i].t() for i in range(8)]  # Transpose for matmul
    
    y0 = (x0@Wt[0] - x1@Wt[1] - x2@Wt[2] - x3@Wt[3] - x4@Wt[4] - x5@Wt[5] - x6@Wt[6] - x7@Wt[7])
    y1 = (x0@Wt[1] + x1@Wt[0] + x2@Wt[3] - x3@Wt[2] + x4@Wt[5] - x5@Wt[4] - x6@Wt[7] + x7@Wt[6])
    y2 = (x0@Wt[2] - x1@Wt[3] + x2@Wt[0] + x3@Wt[1] + x4@Wt[6] + x5@Wt[7] - x6@Wt[4] - x7@Wt[5])
    y3 = (x0@Wt[3] + x1@Wt[2] - x2@Wt[1] + x3@Wt[0] + x4@Wt[7] - x5@Wt[6] + x6@Wt[5] - x7@Wt[4])
    y4 = (x0@Wt[4] - x1@Wt[5] - x2@Wt[6] - x3@Wt[7] + x4@Wt[0] + x5@Wt[1] + x6@Wt[2] + x7@Wt[3])
    y5 = (x0@Wt[5] + x1@Wt[4] - x2@Wt[7] + x3@Wt[6] - x4@Wt[1] + x5@Wt[0] - x6@Wt[3] + x7@Wt[2])
    y6 = (x0@Wt[6] + x1@Wt[7] + x2@Wt[4] - x3@Wt[5] - x4@Wt[2] + x5@Wt[3] + x6@Wt[0] - x7@Wt[1])
    y7 = (x0@Wt[7] - x1@Wt[6] + x2@Wt[5] + x3@Wt[4] - x4@Wt[3] - x5@Wt[2] + x6@Wt[1] + x7@Wt[0])
